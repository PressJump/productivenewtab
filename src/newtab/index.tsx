    ...
    useEffect(() => {
        const fetchCalendarEvents = async () => {
            try {
                ...
                const icalExpander = new ICAL({ ics: icsData, maxIterations: 1000 });
                ...
                const mappedOccurrences = eventsData.occurrences.map((occurrence) => ({
                    summary: occurrence.item.summary,
                    startDate: occurrence.startDate.toJSDate(),
                    endDate: occurrence.endDate.toJSDate(),
                }));

                const parsedEvents = [...mappedEvents, ...mappedOccurrences].sort((a, b) => a.startDate - b.startDate);
    ...
    useEffect(() => {
        const groupByDay = (events: any[]) => {
            ...
            const grouped = days.map((day) => ({
                date: day,
                events: events.filter((event) => new Date(event.startDate).toDateString() === day),
            }));
    ...
    return (
        <div className="p-4 w-full h-screen justify-center items-center flex flex-col">
            {groupedEvents.length === 0 ? (
                ...
                <div className="flex flex-col bg-gray-50 w-fit p-2 rounded-md">
                        ...
                        <div className="p-1.5 inline-block align-middle">
                            ...
                            <table className=" divide-y divide-gray-200 dark:divide-neutral-700">
                                ...
                                <tbody className="divide-y divide-gray-200 dark:divide-neutral-700">
                                    <tr>
                                        {groupedEvents.map((day, index) => (
                                            <td key={index} className="px-2 py-4 whitespace-nowrap text-xs font-medium">
                                                {day.events.length === 0 ? (
                                                    <p>No events</p>
                                                ) : (
                                                    <ul>
