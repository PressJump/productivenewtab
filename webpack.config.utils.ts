...
export const getHTMLPlugins = (
    ...
    new HtmlWebpackPlugin({
        title: 'newtab',
        filename: path.resolve(__dirname, `${outputDir}/${browserDir}/newtab/index.html`),
        template: path.resolve(__dirname, `${sourceDir}/newtab/index.html`),
        chunks: ['newtab'],
    }),
    new HtmlWebpackPlugin({
        title: 'Options',
...
export const getEntry = (sourceDir = Directories.SRC_DIR) => {
    return {
        newtab: [path.resolve(__dirname, `${sourceDir}/newtab/index.tsx`)],
